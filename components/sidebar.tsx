...
export default function Sidebar({ onLogout }: SidebarProps) {
  ...
  return (
    <aside
      className={`hidden lg:flex lg:flex-col fixed top-0 left-0 h-screen border-r border-[#E5E7EB] bg-[#F7F8FA] py-6 z-20 transition-all duration-300 ease-in-out ${
        isCollapsed ? "w-[72px] px-3" : "w-[267px] px-4"
      }`}
    >
      {/* Header */}
      ...
      <div className={`mb-7 ${isCollapsed ? "flex justify-center" : ""}`}>
        <Link
          ...
          className={`group relative flex items-center rounded-xl border transition-colors ${
            isCollapsed ? "h-10 w-10 justify-center" : "h-12 w-full gap-3 px-4"
          } ${
            isActive("/dashboard")
      ...
      <div className={`mt-4 ${isCollapsed ? "flex justify-center" : ""}`}>
        <button
          ...
          className={`group relative flex items-center rounded-lg text-[#B42318] hover:bg-[#FEE4E2] transition-colors ${
            isCollapsed ? "h-10 w-10 justify-center" : "h-10 w-full gap-2 px-3"
          }`}
        >
